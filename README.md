...
| **Proxy clothing does not render** | `Clothing_Torso` and `Clothing_Gloves` on a proxy read `IsVisible()=0` while `Clothing_Legs` and `Clothing_Feet`, same character same frame, read 1. Mesh, bone count, materials, attach parent, leader pose, own visibility flag, component location and actor-space bone positions are all identical to a local player that renders correctly, so whatever hides them is not exposed through reflection. Note `IsVisible()` is false when EITHER the visible flag is clear OR `bHiddenInGame` is set; only the first is `bVisible`, which is why probing `bVisible` alone reported healthy throughout. Ruled out by test: duplicate proxies, the body-part clear, an ancestor hiding the overlay, and setting the character's own `ClothingXEquipped?` flags (that one made it strictly worse and was reverted). Next step is to re-dump `EquipClothingToMesh`/`UpdateBodyParts` on 5.6 via `bytecode_dump.flag` rather than guess again - see the session log. |
