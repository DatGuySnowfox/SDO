...
const VERSION = '1.0.2-public-separated';
...
async function route(req, res) {
  ...
  // Accept both direct upstream paths and paths where Nginx preserved
  // the /profile-api prefix. The canonical public route strips it,
  // but this compatibility prevents another proxy-slash mismatch.
  if (pathname === '/profile-api') {
    pathname = '/';
  } else if (pathname.startsWith('/profile-api/')) {
    pathname = pathname.slice('/profile-api'.length) || '/';
  }
