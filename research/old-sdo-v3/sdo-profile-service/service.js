...
import { validSteamIdentity } from "./bundle.js";
import { PlayerProfileStore, ProfileConflictError } from "./store.js";
export class ProfileService {
    ...
    async handle(request, response) {
        ...
        if (request.method === 'POST' && url.pathname === '/v1/admin/bootstrap') {
            // Server-only mode: admin endpoint to create/seed player profiles
            let input;
            try {
                input = JSON.parse(await this.readBody(request, 48 * 1024 * 1024));
            }
            catch {
                this.sendJson(response, 400, { ok: false, error: 'invalid_request_body' });
                return;
            }
            const bootstrap = input;
            const steamId = bootstrap.steamId;
            const profile = bootstrap.profile;
            if (typeof steamId !== 'string' ||
                !validSteamIdentity(steamId) ||
                !profile ||
            ...
            try {
                // Create profile at revision 0 if it doesn't exist, idempotent
                const record = await this.store.save(steamId, 0, `bootstrap:${steamId}:${Date.now()}`, profile);
                this.sendJson(response, 201, {
                    ...
                    message: 'Player profile bootstrapped on server'
                });
                return;
            }
            catch (error) {
        ...
        const match = /^\/v1\/profiles\/(steam_7656119[0-9]{10})$/.exec(url.pathname);
        if (!match) {
        ...
        if (request.method === 'GET') {
            const record = await this.store.load(steamId);
            if (!record) {
