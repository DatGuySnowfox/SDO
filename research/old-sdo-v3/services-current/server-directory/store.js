...
const GLOBAL_PUBLIC_WORLD_ID = 'global-public-world-pve';
const GLOBAL_DIRECT_GATEWAY_HOST = '134.195.89.248';
const GLOBAL_DIRECT_GATEWAY_PORT = 27930;
const GLOBAL_TUNNEL_GATEWAY_HOST = 'play.sdo.harzgg.space';
const GLOBAL_TUNNEL_GATEWAY_PORT = 443;
const GLOBAL_STREAM_FALLBACK_PORT = 8443;
function defaultGatewayEndpoints(server) {
    const endpoints = [{
            host: GLOBAL_DIRECT_GATEWAY_HOST,
            port: GLOBAL_DIRECT_GATEWAY_PORT
        }];
    if (server.gatewayHost !== GLOBAL_DIRECT_GATEWAY_HOST ||
        server.gatewayPort !== GLOBAL_DIRECT_GATEWAY_PORT) {
        endpoints.push({
            host: server.gatewayHost,
            port: server.gatewayPort
        });
    }
    if (server.id === GLOBAL_PUBLIC_WORLD_ID) {
        endpoints.unshift({
            host: GLOBAL_TUNNEL_GATEWAY_HOST,
            port: GLOBAL_TUNNEL_GATEWAY_PORT
        });
        endpoints.push({
            host: GLOBAL_DIRECT_GATEWAY_HOST,
            port: GLOBAL_STREAM_FALLBACK_PORT
        });
    }
    return endpoints;
}
export function validateRegistry(input) {
    ...
    const servers = value.servers.map((server) => {
        if (!server ||
            ...
            (server.iconUpdatedAtMs !== undefined &&
                (!Number.isSafeInteger(server.iconUpdatedAtMs) || Number(server.iconUpdatedAtMs) < 1))) {
            throw new Error(`server_invalid_${server?.id ?? 'unknown'}`);
        }
        ids.add(server.id);
        const gatewayEndpoints = server.gatewayEndpoints === undefined ||
            server.gatewayEndpoints === null
            ? defaultGatewayEndpoints(server)
            : validateGatewayEndpoints(server.gatewayEndpoints, server.id);
        return { ...server, gatewayEndpoints };
    });
