...
/** Never spawn authoritative zombies closer than this to a living player. */
...
export class HostAgentService {
    ...
    handle(frame) {
        ...
        if (frame.type === MessageType.Movement) {
            ...
            this.capturePlayerProgress(player);
            const relayIntervalMs = Math.max(16, this.config.movementRelayIntervalMs ?? 33);
    ...
    shouldReplayEntityToPlayer(entity, player, now) {
        ...
        if (entity.kind === WorldEntityKind.Zombie) {
            if (this.isWithinPlayerSpawnExclusion(entity, positioned, now)) {
                return false;
            }
    ...
    simulateWorld() {
        ...
        const activationRadiusSquared = Math.max(CLIENT_ZOMBIE_INTEREST_RADIUS, this.config.zombieSpawnerActivationRadius ?? 18_000) ** 2;
        ...
        if (configuredSpawnPoints.length > 0) {
            ...
            if (activePlayers.length > 0) {
                ...
                for (const player of activePlayers) {
                    ...
                    const nearestForPlayer = configuredSpawnPoints
                        ...
                        !this.isWithinPlayerSpawnExclusion(entry.point, player, now))
                ...
                for (const index of activeSpawnerIndices) {
                    ...
                    if (this.isWithinAnyPlayerSpawnExclusion(point, activePlayers, now)) {
                        continue;
                    }
                ...
                if (this.config.zombieExplorationFill !== false) {
                    for (const player of activePlayers) {
                        const inJoinGrace = player.admittedAtMs !== undefined &&
                            now - player.admittedAtMs < this.joinGraceMs();
                        if (inJoinGrace)
                        ...
                        if (nearbyCount >= explorationTarget)
                            continue;
                        const needed = Math.min(explorationTarget - nearbyCount, 2);
                        const forward = this.playerForwardVector(player);
                        for (let slot = 0; slot < needed; slot += 1) {
                            ...
                            const z = player.z;
                            if (this.isWithinPlayerSpawnExclusion({ x, y, z }, player, now)) {
                                continue;
                            }
                            const zombie = {
                                entityId,
            ...
            const baseDistance = Math.max(900, this.config.zombieSpawnDistance ?? 1_400);
            const distance = baseDistance + (serial % 4) * 350;
    ...
    handleZombieAttackRequest(frame) {
        ...
        if (entity.health === 0) {
            ...
            this.worldEntities.delete(entityId);
            const configuredSpawnPoints = this.config.zombieSpawnPoints ?? [];
            const configuredEntity = configuredSpawnPoints.some((_, index) => this.mapSpawnerEntityId(index) === entityId);
            if (configuredEntity) {
                this.configuredZombieRespawnAt.set(entityId, Date.now() + Math.max(60_000, this.config.zombieRespawnMs ?? 600_000));
            }
            else {
                this.nextZombieSpawnAt =
                    Date.now() + Math.max(1_000, this.config.zombieRespawnMs ?? 30_000);
            }
            this.send({
                type: MessageType.EntityDespawn,
                sessionId: this.sessionId,
