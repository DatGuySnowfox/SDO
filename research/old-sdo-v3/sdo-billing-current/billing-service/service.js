...
export class BillingService {
    ...
    constructor(config) {
        ...
        const homePool = config.homePool ?? [];
        if (homePool.some((id) => !SERVER.test(id)))
            throw new Error('billing_home_pool_invalid');
        if (config.plans.some((plan) => plan.hostingMode === 'home-host') && homePool.length < 1) {
            throw new Error('billing_home_pool_invalid');
        }
...
<h1>SD-ONLINE</h1><p>${canceled
        ...
        if (request.method === 'POST' && url.pathname === '/v1/plan-changes') {
            ...
            const currentPlan = this.plan(lease.planId);
            if (currentPlan && currentPlan.hostingMode !== plan.hostingMode) {
                return void this.json(response, 400, { ok: false, error: 'plan_hosting_mode_mismatch' });
            }
    ...
    poolFor(plan) {
        return plan.hostingMode === 'home-host'
            ? (this.config.homePool ?? [])
            : this.config.pool;
    }
    publicLease(lease) {
        const plan = this.plan(lease.planId);
        return {
            serverId: lease.serverId, planId: lease.planId, state: lease.state,
            maxPlayers: plan?.maxPlayers ?? 0,
            hostingMode: plan?.hostingMode ?? 'vps-headless'
        };
    }
