...
const agent = new HostAgentService({
    ...
    movementRelayIntervalMs: Number(process.env.SDO_MOVEMENT_RELAY_INTERVAL_MS ?? 20),
    ...
    zombieRespawnMs: Number(process.env.SDO_ZOMBIE_RESPAWN_MS ?? 7 * 60 * 1000),
    zombieSpawnIntervalMs: Number(process.env.SDO_ZOMBIE_SPAWN_INTERVAL_MS ?? 1_000),
    zombiesPerPlayer: Number(process.env.SDO_ZOMBIES_PER_PLAYER ?? 12),
    zombieMaxCount: Number(process.env.SDO_ZOMBIE_MAX_COUNT ?? 128),
    zombieSpawnDistance: Number(process.env.SDO_ZOMBIE_SPAWN_DISTANCE ?? 5_500),
    zombieSpawnerActivationRadius: Number(process.env.SDO_ZOMBIE_SPAWNER_ACTIVATION_RADIUS ?? 15_000),
    zombieJoinGraceMs: Number(process.env.SDO_ZOMBIE_JOIN_GRACE_MS ?? ZOMBIE_JOIN_GRACE_MS),
    zombieExplorationFill: process.env.SDO_ZOMBIE_EXPLORATION_FILL === '1',
    zombieSpawnPoints,
