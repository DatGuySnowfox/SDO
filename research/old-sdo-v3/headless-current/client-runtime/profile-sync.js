...
import { PROFILE_BUNDLE_VERSION, profileContentSha256, sha256, validProfilePath, validateProfileBundle, isCompleteCanonicalServerBundle, normalizeQuickSaveBundleToSaveSlot, CANONICAL_SERVER_PROFILE_PATHS } from "../profile-service/bundle.js";
...
export class ProfileSync {
    ...
    async recoverPrevious(reason = 'bulkdata-crash', expectedRevision = this.revision) {
        if (this.operation)
            await this.operation;
    ...
    async restoreOrSeed() {
        ...
        const value = await response.json();
        let bundle = validateProfileBundle(value.bundle, this.steamId);
        let revision = value.revision;
        let signature = value.sha256;
        if (this.config.serverCanonical) {
            bundle = normalizeQuickSaveBundleToSaveSlot(bundle, this.steamId);
            if (!isCompleteCanonicalServerBundle(bundle)) {
                try {
                    const recovered = await this.recoverPrevious('canonical-layout-incomplete', value.revision);
                    const retry = await this.request(this.profilePath(), {
                        headers: this.headers(),
                        signal: AbortSignal.timeout(15_000)
                    });
                    if (!retry.ok)
                        throw new Error(`profile_restore_failed_${retry.status}`);
                    const retried = await retry.json();
                    bundle = normalizeQuickSaveBundleToSaveSlot(validateProfileBundle(retried.bundle, this.steamId), this.steamId);
                    if (!isCompleteCanonicalServerBundle(bundle)) {
                        throw new Error('profile_canonical_slot_incomplete');
                    }
                    revision = retried.revision;
                    signature = retried.sha256;
                    log('warning', 'profile_restored_after_layout_recovery', {
                        playerId: this.steamId,
                    ...
                    if (message !== 'profile_recovery_unavailable')
                        throw error;
                    await this.promoteLocalQuickSaveLayoutOnDisk();
                    try {
                        await this.checkpoint('recovery-unavailable-local-seed', true);
                        const retry = await this.request(this.profilePath(), {
                            headers: this.headers(),
                            signal: AbortSignal.timeout(15_000)
                        });
                        if (retry.ok) {
                            ...
                            if (isCompleteCanonicalServerBundle(bundle)) {
                                revision = retried.revision;
                                signature = retried.sha256;
                                log('warning', 'profile_restored_after_local_seed', {
                                    playerId: this.steamId,
                                    saveRevision: revision
                                });
                            }
                        ...
                        // Fall back to joining with a deferred profile restore.
                    }
                    if (!isCompleteCanonicalServerBundle(bundle)) {
                        this.lastError = 'profile_restore_deferred';
                        log('warning', 'profile_restore_deferred_no_recovery', {
                            playerId: this.steamId,
    ...
    async checkpointInner(reason, force) {
        try {
            ...
            if (this.config.serverCanonical &&
                ...
                this.lastError = 'profile_canonical_slot_incomplete';
                log('warning', 'profile_checkpoint_skipped_incomplete_layout', {
                    playerId: this.steamId,
                    saveRevision: this.revision,
                    checkpointReason: reason,
                    fileCount: bundle.files.length,
                    paths: bundle.files.map((file) => file.path).join(',')
                });
                return;
            }
            ...
            if (!response.ok) {
                if (response.status === 422) {
                    this.lastError = 'profile_layout_regression_rejected';
                    log('warning', 'profile_checkpoint_rejected_layout_regression', {
                        playerId: this.steamId,
                        saveRevision: this.revision,
                        checkpointReason: reason
                    });
    ...
    async restoreBundle(bundle) {
        ...
        const normalized = this.config.serverCanonical
            ? normalizeQuickSaveBundleToSaveSlot(bundle, this.steamId)
            : bundle;
        const files = this.config.serverCanonical
            ? normalized.files.filter((file) => this.isCanonicalServerPath(file.path))
            : normalized.files;
        if (files.length === 0)
            throw new Error('profile_canonical_slot_missing');
        if (this.config.serverCanonical &&
            !isCompleteCanonicalServerBundle({
                ...normalized,
                files
            })) {
            throw new Error('profile_canonical_slot_incomplete');
    ...
    isCanonicalServerPath(path) {
        return CANONICAL_SERVER_PROFILE_PATHS.includes(path);
    }
    async promoteLocalQuickSaveLayoutOnDisk() {
        let rootEntries;
        try {
            rootEntries = await readdir(this.config.saveRoot, { withFileTypes: true });
        }
        ...
        const saveSlotRoot = this.safePath('SaveSlot');
        const saveSlotPlayer = join(saveSlotRoot, 'Player.sav');
        try {
            if ((await stat(saveSlotPlayer)).isFile())
                return false;
        }
        catch {
            // SaveSlot/Player.sav is missing; promotion may be required.
        }
        const quickSaveDirectories = rootEntries
            .filter((entry) => entry.isDirectory() && /^QuickSave/.test(entry.name))
            .sort((left, right) => right.name.localeCompare(left.name));
        const quickSave = quickSaveDirectories[0];
        if (!quickSave)
            return false;
        const slotFiles = [
            'Difficulties.sav',
        ...
        await mkdir(saveSlotRoot, { recursive: true });
        for (const name of slotFiles) {
            const source = join(this.config.saveRoot, quickSave.name, name);
            const destination = join(saveSlotRoot, name);
            try {
                await copyFile(source, `${destination}.${process.pid}.promote.tmp`);
                await rename(`${destination}.${process.pid}.promote.tmp`, destination);
            }
            catch {
                return false;
            }
        }
        for (const suffix of ['Quests', 'SkillTree']) {
            const source = join(this.config.saveRoot, `${quickSave.name}_${suffix}.sav`);
            const destination = this.safePath(`SaveSlot_${suffix}.sav`);
            try {
                await copyFile(source, `${destination}.${process.pid}.promote.tmp`);
                await rename(`${destination}.${process.pid}.promote.tmp`, destination);
            }
            catch {
                // Quest and skill-tree companions are optional on older saves.
            }
        }
        log('info', 'profile_quicksave_promoted_before_checkpoint', {
            playerId: this.steamId,
            saveRevision: this.revision,
            quickSaveDirectory: quickSave.name
        });
        return true;
    }
