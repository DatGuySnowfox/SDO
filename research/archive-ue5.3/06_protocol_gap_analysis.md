...
UE5.3 uses large-world coordinates (LWC) with `FVector3d` (3 x double). Casting
...
- Frame header format, magic, version [ok]
- TCP transport + HMAC ticket auth [ok]
- `Movement` encode/decode [ok] including velocity (minor: still float, not double - see gap 10)
- `WorldState` decode [ok]
- `Death`/`Respawn`/`DeathRequest`/`RespawnRequest` flow [ok]
- `PlayerConnected`/`PlayerDisconnected` + proxy lifecycle (position/state tracking only - see below) [ok]
- `PlayerDamage` decode + dispatch [ok] (writes straight to `MedicalComponent`)
- `EntitySpawn`/`EntityState`/`EntityDespawn` [ok] (correct JS-matching formats)
- `ItemPickup`/`ItemDrop`/`Interaction` request+result [ok] (JSON both directions)
- `PlayerProgressRestore` [ok] (correct decode + teleport-on-restore)
- `Equipment` - send (local read) and receive (cache on `RemotePlayer`) [ok], live-confirmed Session 34
- Auto-open world via `MenuWidget_C` [ok]
