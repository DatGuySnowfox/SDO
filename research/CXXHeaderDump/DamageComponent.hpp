...
class UDamageComponent_C : public UActorComponent
{
    ...
    bool CanDamage?;                                                                  // 0x00C0 (size: 0x1)
    double StandardHealthValue;                                                       // 0x00C8 (size: 0x8)
    double CurrentHealth;                                                             // 0x00D0 (size: 0x8)
    double MaxHealth;                                                                 // 0x00D8 (size: 0x8)
    FDamageComponent_CStandardDamage StandardDamage;                                  // 0x00E0 (size: 0x10)
    void StandardDamage(double Dmg, class AActor* DmgCauser);
    FDamageComponent_CDeath Death;                                                    // 0x00F0 (size: 0x10)
    void Death(class AActor* DmgCauser, bool Headshot);
    bool SaveValues?;                                                                 // 0x0100 (size: 0x1)
    FDamageComponent_CSetBurning SetBurning;                                          // 0x0108 (size: 0x10)
    void SetBurning(bool Burning?);
    class UParticleSystemComponent* FireFX;                                           // 0x0118 (size: 0x8)
    FVector2D TimeOfBurning;                                                          // 0x0120 (size: 0x10)
    FTimerHandle TakingDamageTimer;                                                   // 0x0130 (size: 0x8)
    FTimerHandle TimeBurningTimer;                                                    // 0x0138 (size: 0x8)
    FDamageComponent_CSetCurrent&MaxHealth SetCurrent&MaxHealth;                      // 0x0140 (size: 0x10)
    void SetCurrent&MaxHealth(double CurrentHealth, double MaxHealth);
    double HealthRandomDeviation;                                                     // 0x0150 (size: 0x8)
    class USoundBase* DamageSound;                                                    // 0x0158 (size: 0x8)
    class USoundBase* DeathSound;                                                     // 0x0160 (size: 0x8)
    bool CanScaleHealth?;                                                             // 0x0168 (size: 0x1)
    class AActor* CurrentFireDamageGiver;                                             // 0x0170 (size: 0x8)
    bool CanHeadshot1Shot?;                                                           // 0x0178 (size: 0x1)
