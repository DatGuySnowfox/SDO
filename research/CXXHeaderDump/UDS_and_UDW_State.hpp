#ifndef UE4SS_SDK_UDS_and_UDW_State_HPP
#define UE4SS_SDK_UDS_and_UDW_State_HPP

struct FUDS_and_UDW_State
{
    FDateTime CurrentTime_212_A8A3081E448DDADDE3E0AE9D48603236;                       // 0x0000 (size: 0x8)
    FDateTime StartingDate_213_8B4AC69C4C757F9C8F3B599C85B79B12;                      // 0x0008 (size: 0x8)
    FVector CloudPosition_214_9D83C0B849A3B5BFB9C3DD9DA20CE0F7;                       // 0x0010 (size: 0x18)
    FVector SimulationLocation_215_82C5772A4C1B679691D865B904365DF3;                  // 0x0028 (size: 0x18)
    bool IncludesWeatherState_216_B8E4960C4B89E69E587CAD9E453AF85E;                   // 0x0040 (size: 0x1)
    FUDW_WeatherState_Structure ManualWeatherState_217_43B336E740485E3BE53E9FA913B750CE; // 0x0048 (size: 0x50)
    FUDW_WeatherState_Structure GlobalWeatherState_218_ABD7EF5F45579C1C5FEF3B8751C4EB86; // 0x0098 (size: 0x50)
    class UObject* Weather_159_65D3DE2946AD276E7F18E484BA19512B;                      // 0x00E8 (size: 0x8)
    FUDW_WeatherState_Structure OldWeatherState_219_4D413F1E4B69542295AF849DC9D77387; // 0x00F0 (size: 0x50)
    bool TransitionActive_220_BB1BA4AE49DC610D61D5938033CE5706;                       // 0x0140 (size: 0x1)
    double TransitionDuration_221_0ADED53C458AAF00B34A5E84027E329B;                   // 0x0148 (size: 0x8)
    double TransitionTimer_222_D52B116947119ECE68EDEFAF45877D80;                      // 0x0150 (size: 0x8)
    double WindDirection_223_C27727CA49EA2613308F638F36486960;                        // 0x0158 (size: 0x8)
    TArray<class UUDS_Modifier_C*> Modifiers_183_9106119B46CD83CAFE40DAA85329A736;    // 0x0160 (size: 0x10)
    TArray<double> ModifierAlphas_224_3F8ABF2F420E35E9BBCCC5963AED568F;               // 0x0170 (size: 0x10)
    TArray<double> ModifierTargets_225_7340DCC245CD95D843EAC2B94658F829;              // 0x0180 (size: 0x10)
    TArray<double> ModifierSpeeds_226_D00457614E1365607A7DA68178982B63;               // 0x0190 (size: 0x10)
    FRandomWeatherVariation_State UDWRandomWeatherState_227_AE829CEC4D89D738E7D506805258238F; // 0x01A0 (size: 0x40)
    TArray<class AActor*> WOVActors_228_617D1B134B0D80E5D1D5ADBE6FC8A601;             // 0x01E0 (size: 0x10)
    TArray<FUDW_WOV_State> WOVStates_229_7C8393034CE7618138CCB198E6DD1E76;            // 0x01F0 (size: 0x10)

}; // Size: 0x200

#endif
