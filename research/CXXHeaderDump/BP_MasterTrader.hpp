...
class ABP_MasterTrader_C : public ABP_MasterAICharacter_C
{
    ...
    TSubclassOf<class UDialogue> Dialogue;                                            // 0x08A0 (size: 0x8)
    ETextGender Gender?;                                                              // 0x08A8 (size: 0x1)
    TArray<class TSubclassOf<ABP_AIWeapon_Master_C>> RangedWeapon;                    // 0x08B0 (size: 0x10)
    TArray<class TSubclassOf<ABP_AIWeapon_Master_C>> MeleeWeapon;                     // 0x08C0 (size: 0x10)
    bool StartWithRangeWeaponEquipped?;                                               // 0x08D0 (size: 0x1)
    TArray<class USkeletalMesh*> Meshes;                                              // 0x08D8 (size: 0x10)
    TArray<class UStaticMesh*> HairMeshes;                                            // 0x08E8 (size: 0x10)
    TArray<class UStaticMesh*> BeardMeshes;                                           // 0x08F8 (size: 0x10)
    TMap<class UStaticMesh*, class FTransform> BackpackMeshes;                        // 0x0908 (size: 0x50)
    class AActor* CurrentActor;                                                       // 0x0958 (size: 0x8)
    FVector InteractingActorLoc;                                                      // 0x0960 (size: 0x18)
    FText Action Name;                                                                // 0x0978 (size: 0x18)
    class UWidgetComponent* Marker;                                                   // 0x0990 (size: 0x8)
    bool CanAddMarker?;                                                               // 0x0998 (size: 0x1)
    FTimerHandle MarkerTimer;                                                         // 0x09A0 (size: 0x8)
    bool Is Burning?;                                                                 // 0x09A8 (size: 0x1)
    class UParticleSystemComponent* BurningEffect;                                    // 0x09B0 (size: 0x8)
    FTimerHandle BurningTimer;                                                        // 0x09B8 (size: 0x8)
    bool Looted;                                                                      // 0x09C0 (size: 0x1)
    bool CurrentlyInteracting?;                                                       // 0x09C1 (size: 0x1)
