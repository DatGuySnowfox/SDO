...
Tool: IDA Pro 9.3 + Hex-Rays + IDA MCP server (http://<ida-host>:8744/mcp)
...
The MCP server is running on `http://<ida-host>:8744/mcp` (Docker container).
...
| `dword_146EFE180` | `0x146EFE180` | `MaxElements` (= NumChunks x 65536) |
...
| `+0x68` (104) | `SlotDimensions` | StructProperty | Grid size (1x1, 2x1, etc.) |
...
**Confirmed FGuid** - standard UE4 `FGuid` (4 x uint32). All Jigsaw container and
...
### S_ServerEquippedItems - 21 Equipment Slots x 120 bytes each
...
Total: `0x68 + 16 = 0x78 = 120 bytes` [ok]
...
```
Full loadout sync: 21 slots x ~24 bytes = ~504 bytes per player update.
...
```cpp
// RespawnLoc is an FVector3d (3 x double = 24 bytes)
...
| Function | Fired? |
|----------|--------|
| `SERVER_RequestDropItem` | **Yes**, 3x |
| `SERVER_RequestEquipActorToContainer` | **Yes**, 3x |
| `CLIENT_EquipActorSuccess` | **Yes**, 3x |
...
| Function | Fired? |
|----------|--------|
| `SERVER_RequestDropItem` | Yes, 3x (reconfirms Session 27) |
| `JigHelperComp.TryPickup` | **Yes, 2x** |
...
equipped item is its own 1x1 container" - wrong in the specifics, but the fix ended up the same: skip
...
- SSH access configured to both the Windows VM guest and the Proxmox host itself (key-based, matching the
  existing `<ssh-alias>` pattern - see `~/.ssh/config` entries `<pc2-ssh-alias>` and `<proxmox-host>`).
...
- **Local dev server ticket flow is not `play.ps1`'s** (`gateway.example.com`/`/v1/join`, 2-minute TTL,
...
The IDA MCP server (`ida` in `~/.mcp.json`, bridging to `http://<ida-host>:8744/mcp`) wasn't connected to this
...
- **Deprecated `K2_AttachTo` vs. modern `K2_AttachToComponent`** - switched to the non-deprecated 6-param
  version (`EAttachmentRule` x 3 instead of the packed `EAttachLocation::Type` byte); `AttachParent` still NULL.
...
**Also discovered along the way**: after redeploying, PC1 appeared to be stuck in the same "ready -> unloaded x2"
...
**Live reproduction reconfirmed today**: two-client setup (this machine as PC1, `<pc2-hostname>` VM as PC2), PC2
...
Three unique assets across six nodes (rifle's asset reused 3x, presumably one per standing/crouched/other
...
`SkeletalMesh` reference fields, one per sex x body segment (Torso/Arms/Biceps/Hands/Legs/
...
means once a character gets stuck, up to 3 components x 2 `ProcessEvent` calls each
...
bugs found total today (bReinitPose x2 call sites, ScaleRule x1, missing ReturnValue x2 call
...
**Catalog status**: every row is now [done] except two genuinely-minor [partial]s
